...
extension PopupModel {
    ...
    func isValidModel() -> Bool {
        if config == nil
            || popupObj == nil
            || bgView == nil
            || CGSizeEqualToSize(bgView?.bounds.size ?? .zero, .zero) {
            return false
        }
    ...
    func setupCustomViewCorners() {
        ...
        // 没有圆角直接返回
        guard let rectCorners = config?.rectCorners,
              let cornerRadius = config?.cornerRadius else { return }
        ...
        // 没有圆角直接返回
        guard isSetCornet, (config?.rectCorners.rawValue ?? 0) > 0 else { return }
        var cornerRadiusV = contentView
        if let popupObj = popupObj,
           let callback = popupObj.needSetCornerRadiusView,
            let v = callback(),
        ...
        // 没有需要设置的圆角视图直接返回
        guard let cornerRadiusV = cornerRadiusV else { return }
...
extension PopupModel {
    ...
    @objc func timerLoopExecute() {
        guard isValidModel(),
              let popupObj = popupObj else { return }
...
extension PopupModel: UIGestureRecognizerDelegate {
    ...
    func addGesture() {
        ...
        // 上滑手势
        guard (config?.isDragClose ?? false) else { return }
    ...
    @objc func tapBackground(gesture: UIGestureRecognizer) {
        guard (config?.isClickDismiss ?? false),
              let popupObj = popupObj else { return }
    ...
    @objc func dragTipView(gesture: UIPanGestureRecognizer) {
        // 获取手指偏移量
        guard let contentV = contentView,
              let originFrame = originalFrame,
              let popupObj = popupObj else { return }
        let translation = gesture.translation(in: contentV)
    ...
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        if let v = contentView,
           (touch.view?.isDescendant(of: v) ?? false),
...
extension PopupModel {
    ...
    @objc func keyboardWillShow(noti: Notification) {
        ...
        let popVPoint = contentView?.layer.position ?? .zero
        let curMaxY = popVPoint.y + (contentView?.frame.size.height ?? 0) / 2
        let offY = curMaxY - maxY;
        
        // 键盘被遮挡时执行动画
        if maxY < curMaxY {
            let originPoint = contentView?.layer.position ?? .zero
