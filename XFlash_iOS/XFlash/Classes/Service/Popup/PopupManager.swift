...
public class PopupManager {
    ...
    private var waitRemovePool: [PopupModel]
...
public extension PopupManager {
    ...
    func dismiss(popup: PopupProtocol? = nil,
        ...
        if let popup = popup, let model = getPopupBy(popup: popup) {
            dismiss(with: model, isRemoveQueue: false)
        }
...
private extension PopupManager {
    ...
    func pop(with model: PopupModel, isRecover: Bool) {
        ...
        if model.config?.isTerminatorMode ?? false {
            ...
            if allPopupList.count > 0, !isRecover {
                ...
                if (model.config?.priority ?? 0) >= (last.config?.priority ?? 0) {
    ...
    func popAnimate(with model: PopupModel, needAnimation: Bool) {
        ...
        let sceneStyle = model.config?.sceneStyle ?? .center
        switch sceneStyle {
        case .center:
        ...
        case .top:
            ...
            UIView.animate(withDuration: model.config?.popAnimationDuration ?? 0,
                           delay: 0,
        ...
        case .bottom:
            let contentV = model.contentView!
            contentV.layer.position = CGPoint(x: contentV.layer.position.x,
                                              y: CGRectGetMaxY(model.bgView?.frame ?? .zero) + ((model.originalFrame?.size.height ?? 0) * 0.5))
            UIView.animate(withDuration: model.config?.popAnimationDuration ?? 0,
                           delay: 0,
                           usingSpringWithDamping: 1,
                           initialSpringVelocity: 1.0,
    ...
    func handlePopAnimation(with model: PopupModel) {
        guard let contentView = model.contentView else { return }
        
        let startPosition = contentView.layer.position
        let popAnimationType = model.config?.popStyle ?? .fade
        switch popAnimationType {
        case .fade:
            alphaChange(with: model, isPop: true)
        case .fall:
            contentView.layer.position = CGPoint(x: contentView.layer.position.x,
                                                 y: CGRectGetMidY(model.bgView?.frame ?? .zero) - (model.originalFrame?.size.height ?? 0) * 0.5)
        case .rise:
            contentView.layer.position = CGPoint(x: contentView.layer.position.x,
                                                 y: CGRectGetMidY(model.bgView?.frame ?? .zero) + (model.originalFrame?.size.height ?? 0) * 0.5)
        case .scale:
    ...
    func alphaChange(with model: PopupModel, isPop: Bool) {
        guard let bgView = model.bgView else { return }
...
private extension PopupManager {
    ...
    func dismiss(with model: PopupModel, isRemoveQueue: Bool) {
        ...
        let time = DispatchTime(uptimeNanoseconds: UInt64(model.config?.dismissAnimationDuration ?? 0))
    ...
    func dismissAnimate(with model: PopupModel, needAnimation: Bool) {
        ...
        guard needAnimation, let contentV = model.contentView else { return }
...
private extension PopupManager {
    ...
    func getPopupBy(popup: PopupProtocol) -> PopupModel? {
        for m in windowQueue {
            if m.popupObj?.supplyCustomPopupView() == popup.supplyCustomPopupView() {
                return m
            }
        }
        return nil
    ...
    func drawBackgroundView(with model: PopupModel, color: UIColor, alpha: CGFloat) -> UIColor {
        guard !(model.bgView?.hiddenBg ?? false),
